...
function BlockButtonYouMeMix() {
    const images = [
        {
            id: 1,
            img: `${ymmWelcome}`,
            description: 'Make Your Nights Out Easy, Convenient, and Affordable'
        },
        // {
        //     id: 2,
        //     img: `${pass2}`,
    ...
    return (
        <div className="d-grid gap-2" style={{ alignContent: 'center' }}>
            ...
            <div className={showElement ? 'show-element' : 'hide-element'} style={{ left: '51.9%', top: '610px', position: 'absolute', zIndex: 5 }}>

                {images.map((image, index) =>
                    <div>
                        {/* <p style={{ textAlign: 'center', width: '300px', margin: '8px' }}>{image.description}</p> */}
                        <img id='zoom' key={image.id} src={image.img} style={{ display: index === currentIndex ? 'block' : 'none', width: '520px' }} ></img>
                        <p key={image.id} style={{ display: index === currentIndex ? 'block' : 'none', textAlign: 'center', width: '300px', fontSize: '14px', marginLeft: '21%', marginTop: '-10px', color: '#362449' }}>{image.description}</p>

                    </div>
                )}
                {/* <button id='wiggle' onClick={handleNextImage} style={{ width: '13%', height: '70px', marginTop: '-10px', backgroundColor: '#EFEEF5 0% 0% no-repeat padding-box', color: '#A2A3A7', borderColor: 'white', marginLeft: '255px', fontSize: '40px', borderRadius: '50px' }}><NextIcon /></button>
...
//     }
// ]
export default BlockButtonYouMeMix;